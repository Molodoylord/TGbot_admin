...
# Убедимся, что у WEB_APP_URL нет слэша в конце, и добавим путь вебхука
...
async def select_chat_callback(query: CallbackQuery, db_pool: asyncpg.Pool):
    ...
    # Убедимся, что URL приложения указан в переменных
    if not WEB_APP_URL:
...
async def web_app_data_handler(message: Message, db_pool: asyncpg.Pool):
    logger.info("--- [WebApp Data] Получены данные от WebApp ---")
    try:
        admin_id = message.from_user.id
        data_str = message.web_app_data.data
        logger.info(f"Сырые данные: {data_str}")
        
        data = json.loads(data_str)
        action = data.get("action")
        user_id_to_moderate = data.get("user_id")
        chat_id_str = data.get("chat_id")
        logger.info(f"Распарсены: action={action}, user_id={user_id_to_moderate}, chat_id={chat_id_str}")

        if not all([action, user_id_to_moderate, chat_id_str]):
            logger.warning("Неполные данные от WebApp.")
...
async def index_handler(request: web.Request):
    """Отдает главную HTML страницу для Web App."""
    logger.info(f"--- [Web Server] Запрос на главную страницу (index.html) ---")
...
async def webhook_route_handler(request: web.Request):
    try:
        ...
        logger.info(f"--- [Webhook] ПОЛУЧЕН WEBHOOK: {update_data}")
...
if __name__ == "__main__":
    # Проверка ключевых переменных перед запуском
